...
func (s Service) ProcessEvent(event burn_event.BurnEvent) {
	...
	case hedera.StatusIdenticalScheduleAlreadyCreated:
		s.logger.Debugf("[%s] - Scheduled transaction already created - Executing Scheduled Sign for [%s].", event.TxHash, txReceipt.ScheduleID)
		txResponse, err := s.hederaNodeClient.SubmitScheduleSign(*txReceipt.ScheduleID)
		if err != nil {
			s.logger.Errorf("[%s] - Failed to submit schedule sign [%s]. Error: [%s]", event.TxHash, txReceipt.ScheduleID, err)
			return
		}

		txr, err := txResponse.GetReceipt(s.hederaNodeClient.GetClient())
		if err != nil {
			s.logger.Errorf("[%s] - Failed to get transaction receipt for schedule sign [%s]. Error: [%s]", event.TxHash, txReceipt.ScheduleID, err)
			return
		}

		if txr.Status != hedera.StatusSuccess {
			s.logger.Errorf("[%s] - Schedule Sign [%s] failed with [%s].", event.TxHash, txReceipt.ScheduleID, txr.Status)
			return
		}
...
func (s *Service) scheduledTxExecutionCallbacks(txId string) (onSuccess, onFail func()) {
	onSuccess = func() {
		...
		if err != nil {
			s.logger.Errorf("[%s] - Failed to update status completed mined. Error [%s].", txId, err)
			return
		}
	}
