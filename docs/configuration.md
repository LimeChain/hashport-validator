...
`bridge.networks[i].tokens[j].min_amount`                      | ""                                                  | The minimum amount(in the lowest denomination) for the native fungible asset that is allowed to be transferred in both directions. Default is "", which is interpreted as 0.
