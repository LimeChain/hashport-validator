...
func (ew *Watcher) handleMintLog(eventLog *router.RouterMint) {
	...
	targetChainId := ew.evmClient.ChainID()
...
func (ew *Watcher) handleBurnLog(eventLog *router.RouterBurn, q qi.Queue) {
	...
	chain := ew.evmClient.ChainID()

	nativeAsset := ew.mappings.WrappedToNative(eventLog.Token.String(), chain)
	if nativeAsset == nil {
...
func (ew *Watcher) handleBurnERC721(eventLog *router.RouterBurnERC721, q qi.Queue) {
	...
	chain := ew.evmClient.ChainID()
	nativeAsset := ew.mappings.WrappedToNative(eventLog.WrappedToken.String(), chain)
	if nativeAsset == nil {
