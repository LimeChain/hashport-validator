...
`bridge.networks[i].tokens.fungible[j].min_amount`             | ""                                                  | The minimum amount for the native fungible asset that is allowed to be transferred in both directions. Default is "", which is interpreted as 0.
